...
function TandemSpacePage() {
  ...
  const renderRoomCard = (room: TandemSpaceRoomSummary) => (
    <button
      key={room.id}
      type="button"
      className="flex w-full items-start gap-2.5 border-t border-line px-1 py-3 text-left transition-colors first:border-t-0 hover:bg-elevated/40"
    ...
    >
      <div className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-elevated text-text-muted">
        <IonIcon
          ...
          className={room.isPinned ? 'text-[15px] text-warning' : 'text-[15px]'}
        />
      </div>
